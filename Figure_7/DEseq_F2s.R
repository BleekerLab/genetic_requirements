...
# Load data / shape / only keep F2's for DEseq analysis

counts = read.table("Figure_7/abundance_tidy.tsv", header = T, sep = "\t") %>% 
  mutate(target_id = substr(target_id, start = 1, stop = 14)) %>%
  filter(sample %in% c("F2-151", "F2-411", "F2-445",
                       "F2-28", "F2-73", "F2-127"))

counts = pivot_wider(counts, names_from = sample, values_from = est_counts)

# annotations = read.table("Annotations_solyc_numbers.txt", header = T, sep = "\t", fill = T) %>% dplyr::rename(., Sequence_ID = target_id)

sampleinfo <- data.frame(cbind(c("F2_127","F2_151","F2_28","F2_411","F2_445","F2_73"),
                               c("active","lazy","active","lazy","lazy","active")), stringsAsFactors = F)

colnames(sampleinfo) = c("SampleName","Condition")

#shape counts dataset to make it ready for DEseq
counts4DE <- counts
counts4DE[2:7] = lapply(counts4DE[2:7], as.integer)
...
counts4DE <- as.data.frame(counts4DE[which(ExpressionNum > 3),]) #throw away data where more less than 3 samples have less than 10 counts 
counts4DE = dplyr::rename(counts4DE, Sequence_ID = target_id)
counts4DE = distinct(counts4DE, Sequence_ID, .keep_all = TRUE)

counts4DE = counts4DE %>% column_to_rownames(var = "Sequence_ID")
...
res = as.data.frame(res) %>% rownames_to_column(., var = "Sequence_ID") %>% arrange(., -log2FoldChange)

differential = res %>% filter(padj < 0.05)
differential.genes = differential[122:131,1]

# Export for mapman
mapman = res %>% select(Sequence_ID, log2FoldChange) %>% dplyr::rename(IDENTIFIER = Sequence_ID, 
                                                                  VALUE = log2FoldChange)

write.table(mapman, file = "Figure_7/F2_mapman.txt", row.names = FALSE, col.names = TRUE, sep = "\t")

counts4DE.tidy = counts4DE %>% gather(key = "sample",
                                      value = "counts",
                                      -"Sequence_ID")

counts4DE.tidy%>% filter(Sequence_ID =="Solyc01g079170") %>%
  ggplot(aes(x = sample, y = counts))+
  geom_bar(stat = "identity")+
  facet_wrap(~Sequence_ID, scale = "free")
...
write.table(results.annotated, file = "F2_RNAseq_DEseq_resuts_log2FoldChange_no_F1-hab.tsv", sep = "\t", row.names = FALSE)
...
plot(PCD$x, pch = 20, col = as.factor(sampleinfo$Condition, loadings = TRUE))
