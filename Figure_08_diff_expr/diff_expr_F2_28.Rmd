---
title: "Differential expression (F2.28, Elite and PI127826 2020 plants)"
author: "Marc Galland"
date: "10/17/2021"
output:
  pdf_document:
...
```
The density of genes woth low count values (log10 = 0.3) is lower for F2.28 but
seems to be comparable for other genes. 

## Plot log2ratio F2.28 vs Elite
...
A total of **`r nrow(log2ratio)`** have a finite log2ratio in both F2.28 vs Elite and PI127826 vs Elite. 
...
## Write to CSV file
```{r}
dir.create(path = "./tables_F2-28/", showWarnings = F, recursive = T)
write.csv(final, 
          file = "tables_F2-28/diff_res_F2-28_or_PI127826_vs_Elite.csv", 
          row.names = F, 
          quote = F)
```
...
Should be significant (p < 0.05) in F2.28 vs Elite. 

```{r}
signif_genes <- filter(final, pval_P28 < 0.05) %>% pull(gene)
```
...
# show table
mep_mva_gene_signif %>% 
  select(name, gene, pathway, log2ratio_P28, log2ratio_PI127826, pval_P28, pval_PI127826) %>% 
  knitr::kable()

write.csv(mep_mva_gene_signif, 
            file = "tables_F2-28/mep_mva_gene_signif.csv", 
            row.names = F, 
            quote = F)
```
