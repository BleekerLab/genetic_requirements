...
volatiles_VI = read.csv(file = "/Users/Ruy/Documents/Zingiberene & Derivatives/Large F2 EZ/Volatiles UvA/type_VI_gland_zingiberene_F2.csv", header = T, stringsAsFactors = T)
...
#Calculate binwidth
2 * IQR(scale(log(df2$zingiberene))) / length(df2$zingiberene)^(1/3)

p.zingiberene.fullF2 =
ggplot(data = df2, aes(scale(log(zingiberene))))+
  stat_bin(binwidth = 0.2, colour = "grey")+
...
p.hist = 
volatiles_VI %>% filter(!genotype %in% c("PI127826", "CV", "F1")) %>%
...
zingi.scaled = approxfun(density(scale(log(volatiles_VI$zingiberene+1))))

#Calculate the area's of the function: integrate(funtion, xmin, xmax)
integrate(zingi.scaled, -2.3,0.85)
