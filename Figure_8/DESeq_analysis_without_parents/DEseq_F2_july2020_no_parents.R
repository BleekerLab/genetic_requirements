...
counts <- read.table("Figure_8/raw_counts.txt", header = TRUE, sep = "\t", check.names = FALSE) %>%
  mutate(target_id = substr(Geneid, start = 6, stop = 19)) %>% 
   select(target_id, F2_28, F2_73, F2_127, F2_151, F2_411, F2_445)

# Lazy / Active information on the samples
sampleinfo <- data.frame(cbind(colnames(counts[,2:7]),
                               c("active","active","active","lazy","lazy","lazy")), stringsAsFactors = F)

colnames(sampleinfo) <- c("SampleName","Condition")

#Create a df for DESeq2 and add sample names
counts4DE <- counts
colnames(counts4DE)[2:7] <- sampleinfo[,1] # Add samples names 
...
# Export result to txt files
 write.table(res_df, file = "Figure_8/results_without_parents/F2_RNAseq_DEseq_resuts.tsv", sep = "\t", row.names = FALSE)
 write.table(res.significant, file = "Figure_8/results_without_parents/F2_RNAseq_DEseq_resuts_significant_genes.tsv", sep = "\t", row.names = FALSE)
 ...
 ggsave(file = "Figure_8/results_without_parents/volcanoplot.pdf", plot = p.volcano, width = 5, height = 5)
 ...
 con = data.frame(genotype = c("F2_151", "F2_411", "F2_445",
                              "F2_28", "F2_73", "F2_127"),
                  condition = c("lazy","lazy","lazy",
...
# Determine target genes to plot 
 ...
 ggsave(file = "Figure_8/results_without_parents/barplot_top15.pdf", plot = p.barplot.top15, width = 10, height = 10)
...
# Boxplot per condition
 p.boxplot.top15 =
  ...
  labs(x = "Sample" , y = "Gene expression (normalised counts)")+
  my_theme
 ggsave(file = "Figure_8/results_without_parents/boxplot_top15.pdf", plot = p.boxplot.top15, width = 10, height = 10)
...
ggsave(file = "Figure_8/results_without_parents/PCA.pdf", plot = p.pca, width = 5, height = 5)
...
pheatmap(df.for.heatmap,
         ...
         gaps_col = 5,
         filename = "Figure_8/results_without_parents/heatmap_sig_expressed.pdf"
)
