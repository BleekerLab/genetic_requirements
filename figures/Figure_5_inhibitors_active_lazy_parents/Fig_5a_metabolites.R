...
ggsave(filename = "figures/Figure_5_inhibitors_active_lazy_parents/Fig_5A_MVA_MEP terpenes day 14_barplot.pdf", plot = p1, width = 4, height = 5)
...
df.long <- df.long %>% filter(metabolite %in% c("total_MEP_terpenes", "total_MVA_terpenes")) %>% droplevels()
...
# PI127826
df.long.PI = df.long %>% filter(genotype == "PI127826", day == "14")
